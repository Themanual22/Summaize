...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .app{
    ...
    --border:#2a2a2a;--text:#e8e4dc;--muted:#666;
    --danger:#e05252;--green:#52c984;
...
export default function Home() {
  ...
  async function handleSummarize() {
    if (!text.trim() || text.trim().length < 30) {
      setError("Please enter at least 30 characters.");
      return;
    }
    setError("");
    if (!isUnlocked && freeUsed >= FREE_SUMMARY_LIMIT) {
      setShowPaywall(true);
      return;
    }
    setLoading(true);
  ...
  function handleUnlock() {
    ...
    if (code.startsWith("pi_") || code.startsWith("cs_") || code === "PAID" || code.length > 10) {
      ...
      setUnlockError("Invalid code. Complete your Stripe payment first.");
    }
  }
  ...
  return (
    <div className="app">
      <Head>
        ...
        <meta name="description" content="Paste anything. Get the point. AI-powered document summarizer with instant results." />
      ...
      <div className="hero">
        ...
        <p className="hero-sub">
          Drop any document, article, report, or wall of text.
          Get a razor-sharp summary in seconds. No account. No friction.
        </p>
        <div className="hero-actions">
          <button className="btn-primary" onClick={scrollToTool}>Summarize Free →</button>
          <button className="btn-ghost" onClick={() => document.querySelector(".pricing-section")?.scrollIntoView({ behavior: "smooth" })}>
            See Pricing
          </button>
        </div>
      </div>
      ...
      <div className="section">
        ...
        <div className="steps">
          {[
            { n:"01", title:"Paste Your Text", desc:"Drop any content — articles, reports, emails, legal docs, meeting notes. Up to 10,000 characters." },
            { n:"02", title:"Choose Your Mode", desc:"Concise bullets, detailed breakdown, or executive one-liner. You control the output format." },
            { n:"03", title:"Get Your Summary", desc:"Claude processes your text instantly and returns a sharp, structured summary. Copy and go." },
          ].map(s => (
      ...
      <div className="section" ref={toolRef} style={{ paddingTop: 0 }}>
        ...
        <div className="input-wrap">
          <textarea
            ...
            placeholder="Paste your document, article, email, or any text here…"
            value={text}
        ...
        <div className="submit-row">
          ...
          <div className="submit-hint">
            {isUnlocked ? "✓ Full access active" : `${FREE_SUMMARY_LIMIT - freeUsed} free ${FREE_SUMMARY_LIMIT - freeUsed === 1 ? "summary" : "summaries"} remaining`}
          </div>
        </div>
        ...
        {loading && (
          <div className="loading-wrap">
            ...
            <div className="loading-text">Claude is reading your document…</div>
          </div>
        )}
        ...
        {output && !loading && (
          <div className="output-wrap">
            <div className="output-header">
              <div className="output-tag">Summary · {mode}</div>
      ...
      <div className="section pricing-section" style={{ paddingTop: 0 }}>
        ...
        <div className="pricing-card" style={{ opacity: 0.7 }}>
          <div className="pricing-left">
            ...
            <div className="pricing-desc">No payment. No account.</div>
          </div>
          <div className="pricing-features">
            {[`${FREE_SUMMARY_LIMIT} free summaries`,"All 3 modes","Instant output","No signup"].map(f => (
        ...
        <div className="pricing-card">
          <div className="pricing-left">
            <div className="pricing-name">One-Time Access</div>
            <div className="pricing-amount">{PRODUCT_PRICE}</div>
            <div className="pricing-desc">Pay once. Use {PRODUCT_CREDITS}.</div>
          </div>
          <div className="pricing-features">
            {[`${PRODUCT_CREDITS} included`,"All 3 summary modes","No subscription","Stripe secure checkout","Instant access","No account needed"].map(f => (
              <div className="feature-item" key={f}><span className="feature-check">✓</span> {f}</div>
            ))}
          </div>
        ...
        <div className="modal-overlay" onClick={() => setShowPaywall(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-sub">
              Unlock {PRODUCT_CREDITS} instantly — one payment, no subscription, no account.
            </div>
            <div style={{ display: "flex", alignItems: "baseline" }}>
              <div className="price-amount">{PRODUCT_PRICE}</div>
              <span className="price-meta">one-time</span>
            </div>
            <div className="price-includes">Includes {PRODUCT_CREDITS} · All modes · Instant access</div>
            <div className="modal-actions">
              <button className="btn-stripe" onClick={() => window.open(STRIPE_PAYMENT_LINK, "_blank")}>
                Pay {PRODUCT_PRICE} via Stripe →
              </button>
